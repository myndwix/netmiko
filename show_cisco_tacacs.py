#!/usr/bin/env python3
"""
Pull display command(s) from every Cisco switch in cisco_inventory.yaml over
SSH via netmiko, in parallel. Credentials and the commands to run are read
from a config file (cisco_tacacs_config.yaml by default) instead of being
...
def load_config(path):
    ...
    return username, password, secret, show_commands, config_commands


def run_on_host(device_cfg, show_commands, config_commands, username, password, secret):
    device = {
        "device_type": device_cfg.get("device_type", "cisco_ios"),
        "host": device_cfg["host"],
        "username": device_cfg.get("username", username),
        "password": device_cfg.get("password", password),
        "secret": device_cfg.get("secret", secret),
    }
    host = device["host"]
    ...
    outputs = {}
    try:
        if device["secret"] and not conn.check_enable_mode():
            conn.enable()
...
def main():
    ...
    parser.add_argument(
        "--config",
        default="cisco_tacacs_config.yaml",
        help="Path to YAML file with username/password/commands",
    )
    parser.add_argument(
        "--inventory", default="cisco_inventory.yaml", help="Path to inventory YAML file"
    )
    parser.add_argument(
    ...
    username, password, secret, show_commands, config_commands = load_config(args.config)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {
            pool.submit(
                run_on_host, dev, show_commands, config_commands, username, password, secret
            ): dev["host"]
            for dev in devices
        }
